/**
 * Created by Jonibek.Khojibaev on 25.05.2016.
 */
...
var barColor = "#286090";
var finishedBarColor = "#5cb85c";
var movingBarColor = "#286090";
var movingBarOutlineColor = "#888888";
var boxColor = "#DD00DD";
var multiBoxColor = "#286090";
var maxColor = "#FF0000";
...
function newSort() {  // Set up to get ready for a new sort by storing items in random array positions, etc.
    ...
    copyCt = 0;
    valid = false;
    say1("Dasturni boshlash uchun \"Boshlash\" yoki \"Bosqichma bosqich\" tugmalaridan birini tanlang.");
    say2("");
    draw();
...
function drawMax() { // Writes "Max" under one of the items, with an arrow pointing to the item.
    ...
    g.fillText("Katta", x - sw / 2, y + textAscent);
...
function draw() {  // Completely redraws the canvas to show the current state.
    ...
    if (tempOn) {
        g.fillStyle = borderColor;
        var sw = 40;
        g.fillText("Vaqtincha xotira", leftOffset + (16 * barWidth + 15 * barGap - sw) / 2, secondRow_y + 5 + textAscent);
        putItem(0);
    }
    if (maxLoc >= 0)
        drawMax();
